...
from image_features import extract_features_sift_lbp
from image_segmentation import segment_image_mask_rcnn
from classify_images import find_similar_cases_rf
...
class CBR_model:
    ...
    def fit(self, image_folder, label_folder, output_json):
        ...
        class_names = data.get(
            "names", []
        )  # Si "names" no existe, devuelve una lista vacía
        ...
        for file_name in tqdm(common_files):
            ...
            for i, (class_id, x_min, y_min, width, height) in enumerate(bboxes):
                ...
                features = extract_features_sift_lbp(roi)
    ...
    def predict(self, image, image_name, case_database, threshold):
        ...
        bounding_boxes = segment_image_mask_rcnn(image)
        best_case = []
